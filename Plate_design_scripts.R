...
build_min_sample_list <- function(
  ...
  ## 3) Curated lists: Hardcoded priority lists of known, reliable accessions broken down by category
...
create_empty_plate_with_blanks<- function(
  ...
  if (controls_in_samples && !is.null(controls)) {
    warning("You set `controls_in_samples = TRUE` but also provided a `controls` vector. The `controls` argument will be ignored.")
  }
  
  if (controls_in_samples && reps != 1) {
    warning("You set `controls_in_samples = TRUE`, so `reps = ", reps, "` will be ignored.")
  }
  
  if (any(duplicated(sample_list))) {
    warning("Sample list contains duplicate names. Plate design will proceed, but Intertek requires unique identifiers.")
  }
  ...
  if (assay_type=="kasp"){ assay_controls <- 2 }
  if (assay_type=="dart"){ assay_controls <- 2 }
  if (assay_type=="both"){ assay_controls <- 3 }
  
  # Calculate extra sample slots if blanks/controls need to be generated
  if (!controls_in_samples) {
    if (is.null(controls)) {
      stop("You must provide a 'controls' vector if controls_in_samples is FALSE.")
    }
    if (!is.numeric(reps) || reps < 1) {
      stop("'reps' must be a positive integer when controls are to be replicated.")
    }
    replicated_controls <- paste0(rep(controls, each = reps), "_R", rep(1:reps, times = length(controls)))
  ...
  plate_matrix_list <- list()
  plate_list <- data.frame()
  control_counter <- 1
  ...
  for (plates in 1:number_of_plates){
    ...
    if (assay_type=="kasp"){
      temp_plate_list$Sample[grepl("H11", temp_plate_list$Position)] <- control_names[control_counter]
      temp_plate["H","11"] <- control_names[control_counter]
      control_counter <- control_counter + 1
      temp_plate_list$Sample[grepl("H12", temp_plate_list$Position)] <- control_names[control_counter]
      temp_plate["H","12"] <- control_names[control_counter]
      control_counter <- control_counter + 1
    }  
    
    if (assay_type=="dart"){
      temp_plate_list$Sample[grepl("G12", temp_plate_list$Position)] <- control_names[control_counter]
    ...
    plate_list <- rbind(plate_list, temp_plate_list)
  }
  ...
  while (length(unique_ids) < n) {
    new_ids <- replicate(n - length(unique_ids), paste0(sample(LETTERS, 4, replace = TRUE), collapse = ""))
  ...
  for (plate_name in unique(plate_list$Plate)) {
    ...
    tmp_df$Col <- sprintf("%02d", as.numeric(gsub("^[A-Z]", "", tmp_df$Position)))
  ...
  message(sprintf(
    ...
    "Assay Type", assay_print,
    "Samples", sample_number,
    "Plates Required", number_of_plates,
    "Total Blanks", total_controls,
    "Blanks per Plate", assay_controls,
...
#' @param plate_list A data frame representing the plate layout with unique well identifiers, as generated by `create_empty_plate_with_blanks`.
...
#' @return The function updates the global variables `plate_list` and `plate_matrix_list` with the filled sample information and provides a message summarizing the number of samples, empty wells, and any recommendations for filling the remaining wells.
#'
#' 
fill_plate_samples <- function(
  ...
  plate_matrix_list <- plate_lists[['plate_matrix_list']]
  plate_matrix_unique_id <- plate_lists[['plate_matrix_unique_id']]
  ...
  for (plates in 1:length(plate_matrix_list)) {
    ...
    names(tmp_plate_matrix) <- 1:12
    
    tmp_plate_all <- plate_list[plate_list$Plate == temp_name, ]
    
    tmp_plate_samples <- tmp_plate_all[tmp_plate_all$Sample %in% sample_list, ]
    tmp_plate_samples$Column <- as.numeric(gsub("^[A-Z]", "", tmp_plate_samples$Position))
    tmp_plate_samples$Row <- gsub("[0-9]+", "", tmp_plate_samples$Position)
    
    for (cont in 1:nrow(tmp_plate_samples)) {
      row_id <- tmp_plate_samples$Row[cont]
      col_id <- tmp_plate_samples$Column[cont]
      value  <- tmp_plate_samples$Sample[cont]
      tmp_plate_matrix[row_id, col_id] <- value
    }
    
    plate_matrix_list[[temp_name]] <- tmp_plate_matrix
  }
...
summarize_variant_alleles <- function(
  ...
  # --- Load metadata ---
  metadata_csv <-"https://gist.githubusercontent.com/claracruet/e1b1c6de12f11645a527afc7f8d0d12c/raw/49a1d209c4af219226ae32de4b17a1f0258570c3/Sorghum_Metadata_2144g_sm_duplicates_removed_nov2023%2520-%2520Sorghum_Metadata_2144g_sm_duplicates_removed_nov2023.csv"
  ...
  if (length(vcf) == 1) {
    ...
    same_samples <- all(vapply(sample_cols[-1],
                               function(x) identical(x, sample_cols[[1]]),
                               logical(1)))
    
    # If samples differ → stop with a clear message
    if (!same_samples) {
      stop("The VCF files cannot be merged because they do not have identical sample sets or sample order.\n",
    ...
    if (length(dups) > 0) {
      warning(sprintf("Merged VCFs contain duplicate variant IDs (n=%d). First occurrence will be used downstream.", length(dups)))
    }
  }
  ...
  process_one <- function(sel_id) {
    ...
    if (length(idx) == 0) stop(sprintf("Selected variant '%s' not found in VCF.", sel_id))
    if (length(idx) > 1)  warning(sprintf("Selected variant '%s' found %d times; using first.",  sel_id, length(idx)))
    idx <- idx[1]
    ...
    if (grepl(",", ALT, fixed = TRUE)) {
      ...
      warning(sprintf("Multi-ALT site detected (%s). Using first ALT: %s", ALT, ALT_split[1]))
      ALT <- ALT_split[1]
    }
    
    selection_geno <- geno[idx, , drop = FALSE]
    t_selection_geno <- as.data.frame(t(selection_geno), stringsAsFactors = FALSE)
    ...
    haplotype_summary$Allele <- factor(haplotype_summary$Allele, levels = allele_levels, labels = allele_labels)
    
    # Rename Frequency to be unique when merging
    names(haplotype_summary)[names(haplotype_summary) == "Frequency"] <- paste("Frequency", sel_id, sep="_")
    
    # PCV annotation
    haplotype_summary$PCV <- NA
    haplotype_summary$PCV[haplotype_summary$Allele == "Reference"]    <- REF
    haplotype_summary$PCV[haplotype_summary$Allele == "Heterozygous"] <- paste(REF, "/", ALT, sep = "")
    ...
    accessions_variants[, 1] <- factor(accessions_variants[, 1], levels = allele_levels, labels = allele_labels)
    
    # Merge metadata to get "Sample" (and keep LIB)
    accessions_variants <- dplyr::left_join(accessions_variants, metadat, by = c("LIB" = "LIB"))
    ...
    if (isTRUE(write_files)) {
      write.table(accessions_variants, file_accession_hap, sep = "\t", row.names = FALSE, quote = FALSE)
      write.table(haplotype_summary, file_haplotype_freq, sep = "\t", row.names = FALSE, quote = FALSE)
    }
  ...
  if (length(selections) == 1) {
    ...
    for (sel in selections) {
      ...
      if (is.null(haplo_combined)) {
        ...
        haplo_combined <- dplyr::left_join(haplo_combined, tmp, by = "Allele")
      }
    }
    ...
    accessions_combined <- out_list[[first]]$accessions_variants[, c("LIB", "Sample", "PInumber"), drop = FALSE]
    
    # Add each variant’s allele classification
    for (sel in selections) {
      
      tmp <- out_list[[sel]]$accessions_variants[, c("LIB", sel), drop = FALSE]
      accessions_combined <- dplyr::left_join(accessions_combined, tmp, by = "LIB")
    }
...
optimize_plate_params <- function(
  ...
  # Determine max actual samples per plate after reserving mandatory control/blank wells
  if (assay_type == "kasp") assay_controls <- 2
  if (assay_type == "dart") assay_controls <- 2
  if (assay_type == "both") assay_controls <- 3
